...
async function run() {
  try {
    ...
    log();
    log(chalk.green(`🌈 Welcome to Scaffold-ETH 2!`));
    ...
    log(
      chalk.cyan(
        `🚀 Creating a new Scaffold-ETH 2 app in ${chalk.bold(targetPath)}`
      )
    );
    ...
    log(chalk.green(`🌈 Done! Thanks for using Scaffold-ETH 2 🙏`));
    log();
    log(
      chalk.cyan(
